...
def write_graph_in_bytes(graph: nx.Graph, longest_path: list[int]) -> BytesIO:
    ...
    nx.draw_networkx_edges(graph, pos, edge_color="black", width=0.3)
    ...
    nx.draw_networkx_edges(
        graph, pos, edgelist=path_edges, edge_color="darkorange", width=1
    )
...
def get_graph_image(graph: nx.Graph) -> str:
    buffer = write_graph_in_bytes(graph=graph, longest_path=find_longest_path(graph))
