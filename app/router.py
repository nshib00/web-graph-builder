...
async def upload(file: UploadFile = File(...)):
    if file.filename is not None:
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail="Invalid data in Excel file.")
        graphs_table.insert(graph_dict)

        graph = build_graph(graph_dict)
        image_data = get_graph_image(graph)
        return JSONResponse(
            {"image": image_data, "longest_path": find_longest_path(graph)}
        )
...
async def get_graph_by_id(graph_id: str):
    try:
        ...
        image_data = get_graph_image(graph)
        return {"image": image_data, "longest_path": find_longest_path(graph)}
    except ValueError:
